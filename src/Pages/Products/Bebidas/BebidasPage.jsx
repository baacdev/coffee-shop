import React, { useEffect } from 'react';
import { useLocation, Link } from 'react-router-dom';
import Frappuccinos from './Frappuccinos';
import EspressosCalientes from './EspressosCalientes';
import EspressosFrios from './EspressosFrios';
import Refreshers from './Refreshers';
import BebidasCalientes from './BebidasCalientes';
import BebidasFrias from './BebidasFrias';
import ShakensEspressos from './ShakensEspressos';
import { ChevronRightIcon } from '@heroicons/react/20/solid';

function BebidasPage() {
  const location = useLocation();

  useEffect(() => {
    const drinkName = location.pathname.split('/').pop();
    const element = document.getElementById(drinkName);
    if (element) {
      element.scrollIntoView({ behavior: 'smooth' });
    }
  }, [location]);

  return (
    <>
      <div className='lg:col-span-5 min-h-[100vh]'>
        <header className='bg-white lg:border-l border-gray-200'>
          <div className='mx-auto max-w-screen-2xl px-4 sm:px-6 lg:px-8 xl:flex xl:items-center xl:justify-between'>
            <div className='min-w-0 flex-1'>
              <nav className='flex' aria-label='Breadcrumb'>
                <ol role='list' className='flex items-center space-x-4'>
                  <li>
                    <div>
                      <Link to={'/ecommerce/section/menu'} className='text-base font-semibold font-opensans text-gray-500 hover:text-orange-500'>
                        Menú
                      </Link>
                    </div>
                  </li>
                  <li>
                    <div className='flex items-center'>
                      <ChevronRightIcon className='h-5 w-5 flex-shrink-0 text-gray-400' aria-hidden='true' />
                      <a className='ml-4 text-base text-gray-600 font-bold font-opensans'>
                        Bebidas
                      </a>
                    </div>
                  </li>
                </ol>
              </nav>
            </div>
          </div>
        </header>

        <div className='bg-white border-gray-200 lg:border-l'>
          <div id='Frappuccinos' className='mx-auto max-w-screen-2xl px-4 border-gray-200 border-b sm:px-6 lg:px-8 xl:flex xl:items-center xl:justify-between'>
            <h1 className='mt-10 mb-2 text-lg leading-7 font-bold text-gray-900 font-opensans sm:truncate sm:text-xl sm:tracking-tight'>
              FRAPPUCCINOS
            </h1>
          </div>
          {/* FRAPPUCCINOS */}
          <Frappuccinos />

          <div id='espresso-caliente' className='mx-auto max-w-screen-2xl px-4 border-gray-200 border-b sm:px-6 lg:px-8 xl:flex xl:items-center xl:justify-between'>
            <h1 className='mt-10 mb-2 text-lg leading-7 font-bold text-gray-900 font-opensans sm:truncate sm:text-xl sm:tracking-tight'>
              ESPRESSO CALIENTE
            </h1>
          </div>
          {/* ESPRESSO CALIENTE */}
          <EspressosCalientes />

          <div id='espresso-frio' className='mx-auto max-w-screen-2xl px-4 border-gray-200 border-b sm:px-6 lg:px-8 xl:flex xl:items-center xl:justify-between'>
            <h1 className='mt-10 mb-2 text-lg leading-7 font-bold text-gray-900 font-opensans sm:truncate sm:text-xl sm:tracking-tight'>
              ESPRESSO FRIO
            </h1>
          </div>
          {/* ESPRESSO FRIO */}
          <EspressosFrios />

          <div id='refreshers' className='mx-auto max-w-screen-2xl px-4 border-gray-200 border-b sm:px-6 lg:px-8 xl:flex xl:items-center xl:justify-between'>
            <h1 className='mt-10 mb-2 text-lg leading-7 font-bold text-gray-900 font-opensans sm:truncate sm:text-xl sm:tracking-tight'>
              REFRESHERS
            </h1>
          </div>
          {/* REFRESHERS */}
          <Refreshers />

          <div id='otras-bebidas-calientes' className='mx-auto max-w-screen-2xl px-4 border-gray-200 border-b sm:px-6 lg:px-8 xl:flex xl:items-center xl:justify-between'>
            <h1 className='mt-10 mb-2 text-lg leading-7 font-bold text-gray-900 font-opensans sm:truncate sm:text-xl sm:tracking-tight'>
              BEBIDAS CALIENTES
            </h1>
          </div>
          {/* BEBIDAS CALIENTES */}
          <BebidasCalientes />

          <div id='otras-bebidas-frias' className='mx-auto max-w-screen-2xl px-4 border-gray-200 border-b sm:px-6 lg:px-8 xl:flex xl:items-center xl:justify-between'>
            <h1 className='mt-10 mb-2 text-lg leading-7 font-bold text-gray-900 font-opensans sm:truncate sm:text-xl sm:tracking-tight'>
              BEBIDAS FRIAS
            </h1>
          </div>
          {/* BEBIDAS FRIAS */}
          <BebidasFrias />

          <div id='shaken-espresso' className='mx-auto max-w-screen-2xl px-4 border-gray-200 border-b sm:px-6 lg:px-8 xl:flex xl:items-center xl:justify-between'>
            <h1 className='mt-10 mb-2 text-lg leading-7 font-bold text-gray-900 font-opensans sm:truncate sm:text-xl sm:tracking-tight'>
              SHAKEN'S ESPRESSOS
            </h1>
          </div>
          {/* SHAKEN'S ESPRESSOS */}
          <ShakensEspressos />
        </div>
      </div>
    </>
  );
}

export default BebidasPage;
